...
export default function Dashboard() {
  ...
  const getUserTypeLabel = () => {
    if (user.userType === 'subscription') return '订阅会员';
    if (user.userType === 'pay_per_use') return '按次付费';
    return '免费用户';
  };
    ...
    <main className="min-h-screen" style={{ background: 'linear-gradient(135deg, #0f172a 0%, #0c1a3a 50%, #0f2027 100%)' }}>
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <a href="/" className="text-slate-400 hover:text-white transition-colors text-sm">← 返回首页</a>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          ...
          <div className="rounded-xl p-5" style={{ background: 'rgba(15,23,42,0.9)', border: '1px solid rgba(56,189,248,0.12)' }}>
            <div className="text-slate-400 text-sm mb-2">剩余次数</div>
          ...
          {user.subscriptionEndDate && (
            <div className="rounded-xl p-5" style={{ background: 'rgba(15,23,42,0.9)', border: '1px solid rgba(56,189,248,0.12)' }}>
              <div className="text-slate-400 text-sm mb-2">订阅到期</div>
          ...
          {/* Standards */}
          <div className="rounded-xl p-5" style={{ background: 'rgba(15,23,42,0.9)', border: '1px solid rgba(56,189,248,0.12)' }}>
            <div className="text-slate-400 text-sm mb-2">标准切换</div>
            <div className="text-lg font-semibold text-white">
              {user.canSwitchStandards ? '✅ 不限' : '❌ 受限'}
            </div>
          </div>
        </div>
        ...
        <div className="rounded-2xl p-6" style={{ background: 'rgba(15,23,42,0.9)', border: '1px solid rgba(56,189,248,0.12)' }}>
          <h3 className="text-xl font-semibold text-white mb-4">升级套餐</h3>
